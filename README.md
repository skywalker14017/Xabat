...
- Built to fail *safe*: if the scanner can't run, images get blocked by default instead of waved through

**It isn't:**
- Law enforcement, a courtroom, or a guaranteed outcome
- A place to upload explicit images or CSAM (Child Sexual Abuse Material): screenshots get scanned automatically, and anything flagged is rejected before a human ever sees it
- Perfect: the scanner can miss things, the age-reference data is a screening signal and not a legal ruling, and moderators are still just people
...
- 🔍 **Automatic NudeNet image screening** on every attachment, before it ever reaches a moderator
- ✅ **Human triage queue**: mods approve or nuke each image individually before it becomes case evidence
- 🧵 **Dedicated case threads**, plus a private conversation thread/DM relay between mods and the reporter
- 🌍 **Jurisdiction-aware screening flags**: cross-checks reported ages against a large country-by-country age-of-consent reference table, purely to flag "a human needs to look at this," never as a legal verdict
...
```mermaid
flowchart TD
    A[Member runs /report] --> B[Report posted to secure mod channel]
    B --> C[Bot DMs reporter to collect screenshots]
    C --> D[Image uploaded]
    D --> E{NudeNet scan}
    E -- flagged or scan failed --> F[Auto-rejected, never stored or forwarded]
    E -- clear --> G[Sent to moderator to triage queue]
    G --> H{Moderator review}
    H -- approve --> I[Moved into the case's evidence thread]
    H -- reject --> F
    B --> J[Moderator sets case status]
    J --> K[Under Review / Resolved / False Report]
    K --> L[Reporter notified, audit log entry written]
```

The important bit is step E: if NudeNet is missing, times out, or throws an error, the image is treated as explicit and blocked. It fails safe, not open.
...
Moderators also get button controls right on each report and image: **Approve & Move**, **Delete (Illegal/Nuke)**, **Under Review**, **Resolved**, **False Report**.
...
- You cannot strip, disable, or weaken the safety mechanisms (image scanning, retention limits, access controls) and call it a fork. That's an instant, non-curable license termination
- Using Xabat to facilitate grooming, CSAM, or anything it exists to stop works the same way: instant termination, no grace period
...
- **What if I upload something explicit by accident?** It gets auto-screened and rejected before any moderator sees it.
